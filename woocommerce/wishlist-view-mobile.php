...
<ul class="shop_table cart wishlist_table wishlist_view responsive mobile <?php $show_cb ? esc_attr_e('with-checkbox', 'streamit') : ''; ?> <?php $no_interactions ? esc_attr_e('no-interactions', 'streamit') : ''; ?>" data-pagination="<?php echo esc_attr($pagination); ?>" data-per-page="<?php echo esc_attr($per_page); ?>" data-page="<?php echo esc_attr($current_page); ?>" data-id="<?php echo esc_attr($wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
	...
	?>
					...
					<div class="item-wrapper">
						...
						<div class="item-details">
							<div class="product-name">
								<span><?php echo  esc_html__('Product Name:', 'streamit') ?></span>
								<a href="<?php echo esc_url(get_permalink(apply_filters('woocommerce_in_cart_product', $item->get_product_id()))); ?>">
							...
							<?php if ($show_variation || $show_dateadded || $show_price) : ?>
								<table class="item-details-table">
									...
									<?php if ($show_dateadded && $item->get_date_added()) : ?>
										<tr>
											<td class="label">
												<?php esc_html_e('Added on:', 'streamit'); ?>
											</td>
											<td class="value">
						...
						<?php if ($show_quantity || $show_stock_status) : ?>
							<table class="additional-info">
								<?php if ($show_quantity) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e('Quantity:', 'streamit'); ?>
										</td>
										<td class="value">
								...
								<?php if ($show_stock_status) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e('Stock:', 'streamit'); ?>
										</td>
										<td class="value">
											<?php echo 'out-of-stock' === $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html__('Out of stock', 'streamit') . '</span>' : '<span class="wishlist-in-stock">' . esc_html__('In Stock', 'streamit') . '</span>'; ?>
										</td>
									</tr>
								<?php endif ?>
							</table>
						...
						<div class="d-flex justify-content-between align-items-center">
							...
							<?php if ($show_remove_product || $repeat_remove_button) : ?>
								<div class="product-remove">
									<a href="<?php echo esc_url($item->get_remove_url()); ?>" class="remove_from_wishlist" title="<?php echo esc_html(apply_filters('yith_wcwl_remove_product_wishlist_message_title', __('Remove this product', 'streamit'))); ?>"><i class="far fa-trash-alt"></i></a>
								</div>
							<?php endif; ?>
						</div>
		...
		<p class="wishlist-empty">
			<?php echo esc_html(apply_filters('yith_wcwl_no_product_to_remove_message', __('No products added to the wishlist', 'streamit'))); ?>
		</p>
	<?php endif; ?>
