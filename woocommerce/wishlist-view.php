...
<table class="shop_table cart wishlist_table wishlist_view traditional responsive <?php $no_interactions ?  esc_attr_e('no-interactions','streamit') : ''; ?> <?php $enable_drag_n_drop ? esc_attr_e('sortable','streamit') : ''; ?> " data-pagination="<?php echo esc_attr($pagination); ?>" data-per-page="<?php echo esc_attr($per_page); ?>" data-page="<?php echo esc_attr($current_page); ?>" data-id="<?php echo esc_attr($wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
	...
	<thead>
		<tr>
			...
			<th class="product-name">
				<span class="nobr">
					<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_name_heading', __('Product name', 'streamit'), $wishlist)); ?>
				</span>
			</th>
			...
			<?php if ($show_price || $show_price_variations) : ?>
				...
				<th class="product-price">
					<span class="nobr">
						<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_price_heading', __('Unit price', 'streamit'), $wishlist)); ?>
					</span>
				</th>
			<?php endif; ?>
			...
			<?php if ($show_quantity) : ?>
				...
				<th class="product-quantity">
					<span class="nobr">
						<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_quantity_heading', __('Quantity', 'streamit'), $wishlist)); ?>
					</span>
				</th>
			<?php endif; ?>
			...
			<?php if ($show_stock_status) : ?>
				...
				<th class="product-stock-status">
					<span class="nobr">
						<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_stock_heading', __('Stock status', 'streamit'), $wishlist)); ?>
					</span>
				</th>
			<?php endif; ?>
			...
			<?php if ($enable_drag_n_drop) : ?>
				...
				<th class="product-arrange">
					<span class="nobr">
						<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_arrange_heading', __('Arrange', 'streamit'), $wishlist)); ?>
					</span>
				</th>
			<?php endif; ?>
	...
	<tbody class="wishlist-items-wrapper">
		...
		?>
					<tr id="yith-wcwl-row-<?php echo esc_attr($item->get_product_id()); ?>" data-row-id="<?php echo esc_attr($item->get_product_id()); ?>">
						...
						<?php if ($show_remove_product) : ?>
							<td class="product-remove">
								<div>
									<a href="<?php echo esc_url($item->get_remove_url()); ?>" class="remove remove_from_wishlist" title="<?php echo esc_html(apply_filters('yith_wcwl_remove_product_wishlist_message_title', __('Remove this product', 'streamit'))); ?>"><i class="far fa-trash-alt"></i></a>
								</div>
							</td>
						<?php endif; ?>
						...
						<td class="product-name">
							...
							<a class="streamit-product-title" href="<?php echo esc_url(get_permalink(apply_filters('woocommerce_in_cart_product', $item->get_product_id()))); ?>">
						...
						<?php if ($show_stock_status) : ?>
							<td class="product-stock-status">
								...
								<?php echo 'out-of-stock' === $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html(apply_filters('yith_wcwl_out_of_stock_label', __('Out of stock', 'streamit'))) . '</span>' : '<span class="wishlist-in-stock">' . esc_html(apply_filters('yith_wcwl_in_stock_label', __('In Stock', 'streamit'))) . '</span>'; ?>
						...
						<?php if ($show_last_column) : ?>
							<td class="product-add-to-cart">
								...
								if ($show_dateadded && $item->get_date_added()) :
									...
									echo '<span class="dateadded">' . esc_html(sprintf(__('Added on: %s', 'streamit'), $item->get_date_added_formatted())) . '</span>';
								endif;
								?>
								...
								<?php if ($show_add_to_cart && isset($stock_status) && 'out-of-stock' !== $stock_status) : ?>
									<?php
									echo	streamit()->streamit_get_comment_btn($tag = "a",  $label = esc_html__('View Product', 'streamit'), $show_icon = false, $attr = array(
								...
								<?php if ($move_to_another_wishlist && $available_multi_wishlist && count($users_wishlists) > 1) : ?>
									<?php if ('select' === $move_to_another_wishlist_type) : ?>
										<select class="change-wishlist selectBox">
											<option value=""><?php esc_html_e('Move', 'streamit'); ?></option>
											<?php
									...
									<?php else : ?>
										<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
											<?php echo esc_html(apply_filters('yith_wcwl_move_to_another_list_label', __('Move to another list &rsaquo;', 'streamit'))); ?>
										</a>
									<?php endif; ?>
								...
								<?php if ($repeat_remove_button) : ?>
									<a href="<?php echo esc_url($item->get_remove_url()); ?>" class="remove_from_wishlist button" title="<?php echo esc_html(apply_filters('yith_wcwl_remove_product_wishlist_message_title', __('Remove this product', 'streamit'))); ?>"><?php esc_html_e('Remove', 'streamit'); ?></a>
								<?php endif; ?>
			...
			?>
			<tr>
				<td colspan="<?php echo esc_attr($column_count); ?>" class="wishlist-empty"><?php echo esc_html(apply_filters('yith_wcwl_no_product_to_remove_message', __('No products added to the wishlist', 'streamit'), $wishlist)); ?></td>
			</tr>
		<?php
		endif;
